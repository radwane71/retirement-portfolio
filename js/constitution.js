...
// ══════════════════════════════════════════════════════════════════════
// ⚠️ المفاتيح هنا يجب أن تطابق `OFFICIAL_SECTORS` في `js/tickerdb.js`
// حرفاً بحرف، لأن `r.sector` يأتي من هناك. وكانت ستة مفاتيح لا تطابق أي
// قطاع رسمي («البتروكيماويات» و«الأسمنت» — والرسمي «المواد الاساسية» —
// و«التجزئة الكمالية» و«تجزئة الأغذية» و«الاستثمار والتمويل»، وأخطرها
// «الاتصالات» بألف وصل مقابل «الإتصالات» الرسمية بهمزة قطع: فارق حرف
// واحد). فكانت سابك والأسمنت والاتصالات والريتات كلها تسقط إلى 0.45،
// وثلاثة عشر قطاعاً رسمياً بلا معامل — فيخرج إفصاح م.30 أدنى بوضوح من
// «~70%» المذكورة في الدستور. إفصاح فقط، لا يولّد إشارة بيع (م.9).
const GOV_EXPOSURE = {
  'الطاقة': 1.00,
  'المرافق العامة': 1.00,
  'المواد الاساسية': 0.80,              // بتروكيماويات + أسمنت + تعدين
  'السلع الرأسمالية': 0.70,
  'البنوك': 0.70,
  'الخدمات المالية': 0.60,
  'إدارة وتطوير العقارات': 0.60,
  'الصناديق العقارية المتداولة': 0.60,
  'الإتصالات': 0.60,
  'الخدمات التجارية والمهنية': 0.60,
  'النقل': 0.55,
  'الرعاية الصحية': 0.55,
  'الأدوية': 0.50,
  'التأمين': 0.50,
  'التطبيقات وخدمات التقنية': 0.50,
  'الإعلام والترفيه': 0.45,
  'تجزئة وتوزيع السلع الكمالية': 0.45,
  'تجزئة وتوزيع السلع الاستهلاكية': 0.45,
  'الخدمات الإستهلاكية': 0.45,
  'السلع طويلة الأجل': 0.45,
  'المنتجات المنزلية و الشخصية': 0.40,
  'إنتاج الأغذية': 0.40,
  'صناديق المؤشرات': 0.45,
  'أخرى': 0.45,
};

// أسماء قديمة أو مُدخَلة يدوياً — تُردّ إلى القطاع الرسمي بدل أن تسقط
const GOV_ALIASES = {
  'البتروكيماويات': 'المواد الاساسية',
  'الأسمنت': 'المواد الاساسية',
  'التعدين': 'المواد الاساسية',
  'التجزئة الكمالية': 'تجزئة وتوزيع السلع الكمالية',
  'تجزئة الأغذية': 'تجزئة وتوزيع السلع الاستهلاكية',
  'الاستثمار والتمويل': 'الخدمات المالية',
};
const GOV_DEFAULT = 0.45;   // قطاع غير مُدرَج — يُعلَن أنه افتراضي

// توحيد كتابة الهمزات والتطويل حتى لا يُسقط فارقُ حرفٍ قطاعاً كاملاً
function _govNormSector(s) {
...
const _GOV_NORM = (() => {
  const m = {};
  Object.keys(GOV_EXPOSURE).forEach(k => { m[_govNormSector(k)] = GOV_EXPOSURE[k]; });
  Object.keys(GOV_ALIASES).forEach(k => {
    const t = GOV_EXPOSURE[GOV_ALIASES[k]];
...
function govExposure(rows) {
  ...
  (rows || []).forEach(r => {
    ...
    const f = _GOV_NORM[_govNormSector(sec)];
...
const DIV_GROWTH_WEIGHTED  = 0.0276;   // م.7 — نمو التوزيع المرجّح المسجَّل
const ASSUMED_YIELD       = 0.055;     // م.7 — العائد الذي يُشتقّ منه GOAL_PORTFOLIO
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    DIV_GROWTH_WEIGHTED, ASSUMED_YIELD,
