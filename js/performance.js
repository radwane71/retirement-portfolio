...
// الحلّ: تخزين **الوعد** لا النتيجة، فيتشارك المتزامنون نفس العملية. ويُخزَّن
// على الدالة نفسها لا في متغيّر عام، فتبقى مستقلّة قابلة للاستخراج وحدها.
async function loadEntryValuations() {
  if (_entryVal) return _entryVal;
  if (loadEntryValuations._inflight) return loadEntryValuations._inflight;
  loadEntryValuations._inflight = (async () => {
  const _entryVal = {};
  let hist = null;
  ...
  const _res = await loadEntryValuations._inflight;
  _entryVal = _res;                       // الإسناد **بعد** الاكتمال لا قبله
  loadEntryValuations._inflight = null;
  return _res;
}
...
function _stockFlows(coveredSet) {
  ...
  if (coveredSet) {
    ...
    // ══════════════════════════════════════════════════════════════════
    // م.22 — أسهم المنحة إجراء شركة **لا عائد**
    // ------------------------------------------------------------------
    // السلسلة السعرية (Yahoo close) مُعادة التسوية بأثر رجعي لإجراءات
    // الشركة، بينما `_sharesAtISO` يقفز يوم المنحة ولا يُعاد بيان ما قبله.
    // فيجتمع في نقطة واحدة: سعرٌ مُعاد بيانه وعددُ أسهمٍ ليس كذلك — فتُبخَس
    // كل نقاط ما قبل المنحة، ثم تقفز القيمة يوم المنحة قفزةً **بلا تدفّق
    // مقابل**، فيقرأها TWR ربحاً ويُبقيها في المعامل التراكمي إلى آخر
    // السلسلة، وتتسرّب إلى التذبذب وشارب وسورتينو وأقصى التراجع.
    // قياس: منحة الرياض 1:3 ⇒ **+33.3%** ربحاً وهمياً على مركزٍ عائده صفر،
    // والإنماء (2.5→3.0 مليار) ⇒ +20%. وم.22 تسمّي الحدثين نصّاً.
    // العلاج: قيمتها السوقية يوم المنحة تدفّقٌ داخل، فيصير عائد ذلك اليوم
    // سعرياً بحتاً — نفس نمط «دخول السهم إلى نطاق القياس» أعلاه.
    // (مسار **التجزئة** سليم أصلاً: صفوف الدفتر تُعاد كتابتها بالمعامل.)
    // ══════════════════════════════════════════════════════════════════
    (_tx || []).forEach(t => {
      if (t.type !== 'grant' || !t.date) return;
      if (!coveredSet.has(t.ticker)) return;
      const arr = _priceMapOf(t.ticker);
      if (!arr || !arr.length) return;
      const from = _firstPriceDateOf(t.ticker);
      if (from && t.date < from) return;          // خارج نطاق القياس أصلاً
      let px = null;
      for (const [d, c] of arr) { if (d <= t.date) px = +c; else break; }
      if (px == null) px = +arr[0][1];
      const mv = (+t.shares || 0) * px;
      if (mv > 0) out.push({ date: t.date, type: 'deposit', amount: mv,
                             synthetic: true, ticker: t.ticker, grant: true });
    });
  }
  return out;
}
...
function renderReturns() {
  ...
  // ⚠️ فُصلت الكتلة قسمين (2026-08-27، بطلب المالك «حطها minimized»):
  //   • **المنهجية** — تعليمٌ ثابت لا يتغيّر بتغيّر بياناتك ⇒ مطويّة.
  //   • **الإفصاحات** — ما خرج من حسابك أنت (أسهم بلا أسعار، دخول
  //     متأخّر، نقد راكد) ⇒ تبقى ظاهرة. إخفاء إفصاحٍ خلف نقرة يجعله
  //     تقديراً صامتاً (م.20)، وهو نقيض غرضه.
  const method = foldHtml('كيف يُحسب هذا الرقم؟',
      `<strong>ما يدخل في الرقم:</strong> ارتفاع الأسعار <em>و</em>التوزيعات النقدية `
    + `<em>و</em>الأرباح المحقَّقة من البيع — عائدٌ إجمالي كامل. `
    + `<strong>وأسهم المنحة ليست عائداً</strong>: هي إجراء شركة يزيد العدد ويخفض `
    + `السعر بالنسبة نفسها، فتدخل قيمتها تدفّقاً داخلاً ولا تُقرأ ربحاً (م.22). `
  ...
  const disclosures = [
    (missing.length
        ? `⚠️ <strong>خارج TWR:</strong> ${esc(missing.join('، '))} — لا أسعار تاريخية لها، `
    ...
    + ((d.xirrExcluded && d.xirrExcluded.length)
        ...
        : ''),
  ].filter(Boolean).join('').replace(/^(?:<br>)+/, '');   // بندٌ أول غائب يترك <br> يتيماً

  // الإفصاح بحالة `warn` لا `info`: هو تنبيه على نقص في قياسِك أنت،
  // وطبقة التنقية تستثني `.note[data-state]` فلا يُخفى مع الشروح.
  const notes = method + (disclosures ? noteHtml('⚠️', disclosures, 'warn') : '');
