...
// وبلا التزامات. اليدوية (هذه الصفحة) تسجّل صافي الثروة الكامل.
...
window.CARD_INFO = {
  ...
  'nw-history': {
    ...
    body: `
      ...
      <div class="info-math">
        ✎ <strong>اللقطة اليدوية</strong> (من هذه الصفحة) = صافي الثروة الكامل: كل الأصول − كل الالتزامات.<br>
...
function renderChart() {
  ...
  if (noteEl) {
    noteEl.innerHTML = (autoN && manualN)
      ? noteHtml('⚠️', '<b>سلسلتان مختلفتان:</b> ✎ اليدوية = صافي الثروة الكامل (بعد خصم الالتزامات) · ✦ التلقائية = أسهم + نقد + عقار فقط. لا تُقارَن نقطة من هذه بنقطة من تلك.', 'warn')
      : autoN
  ...
  const tooltipCb = {
    ...
    afterLabel: c => {
      ...
      return s ? (isAutoSnap(s) ? '✦ تلقائية — أسهم + نقد + عقار' : '✎ يدوية — صافي الثروة الكامل') : '';
    }
  };
  const baseOpts = {
...
function renderSnapshotTable() {
  ...
  tbody.innerHTML = order.map(i => {
    ...
    return `<tr>
      <td title="${auto ? 'لقطة تلقائية من لوحة التحكم: أسهم + نقد + عقار فقط' : 'لقطة يدوية: صافي الثروة الكامل بعد خصم الالتزامات'}">
...
function exportNetworthCSV() {
  ...
  // التلقائية تقيس (أسهم+نقد+عقار) واليدوية تقيس صافي الثروة الكامل.
