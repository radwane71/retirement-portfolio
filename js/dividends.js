...
function _projectedAnnualIncome() {
  ...
  heldTickers.forEach(ticker => {
    ...
    applyGrantRestatement(dpsSeries, grantRestateFactors(
      (txBuyRows || []).filter(x => x.ticker === ticker && x.type === 'grant'),
      d => _sharesAtDate(ticker, d)));
...
function renderYearlySummary({ yearPortfolio }, fwdPrecomputed) {
  ...
  ${foldHtml('فرق الأعمدة الثلاثة', `🔄 <strong>السنة الجارية</strong>: عائد جزئي فعلي حتى اليوم (${daysElapsed}/${daysInYear} يوم) — ليس مقياس الأداء الأنسب للسنة غير المكتملة |
    ▶ <strong>Forward</strong>: الأدق — آخر دفعة لكل سهم × دوريتها × الأسهم الحالية (مثل ياهو فاينانس) |
    السنوات المنتهية: أرباح فعلية ÷ رأس المال أول يناير`, { icon: '🔄' })}`;
}
...
function renderDividendQuality() {
  ...
  el.innerHTML = `
    ...
    ${foldHtml('كيف تُقرأ درجات الجدول؟', `الدرجات مبنية على <strong>نوافذ 12 شهراً متحرّكة</strong> مرساتها آخر توزيعة (لا السنة التقويمية) — فتتحرك الدرجة فور اكتمال نافذة، بلا انتظار رأس السنة.<br>
      ...
      آخر توزيع = المبلغ الفعلي المُستلَم (ليس DPS)`, { icon: '📐' })}
  `;
}
