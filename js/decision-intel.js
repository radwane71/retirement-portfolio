...
window.DecisionIntel = (function () {
  ...
  function scoreCandidate(x, thr) {
    ...
    const gates = [];
    // ══════════════════════════════════════════════════════════════════
    // ثلاث بوابات ملزِمة كانت غائبة — وطبقة الذكاء هذه تقول «لو أنا مكانك:
    // أوجّه التوزيعة القادمة إلى…» على **الشاشة نفسها** التي تطبّق فيها
    // بانية الخطة هذه البوابات. فكان المخرَجان يتناقضان أمام المالك.
    //   م.12: «ممنوع اقتراحهما أو إدراجهما في أي مخرَج، تحت أي ظرف، ولو
    //         استوفيا كل الفلاتر».
    //   م.55/5: سدافكو 2270 «لا تجميع مهما كانت الإشارات» — وهي حيازة
    //         قائمة فعلاً، فالحالة ليست نظرية.
    //   م.53/4 و م.55/2: سهمٌ في قائمة الخروج المؤجل لا يتلقّى سيولة.
    // ══════════════════════════════════════════════════════════════════
    if (typeof isBanned === 'function' && isBanned(r.ticker))
      gates.push('استبعاد دائم (م.12) — ممنوع في أي مخرَج تحت أي ظرف');
    if (typeof isNoAccumulate === 'function' && isNoAccumulate(r.ticker))
      gates.push('لا تجميع بقرارك الصريح (م.55/5) — مهما كانت الإشارات');
    if (typeof deferredExits === 'object' && deferredExits && deferredExits[r.ticker])
      gates.push('في قائمة الخروج المؤجل (م.53/4 و م.55/2) — لا سيولة جديدة');
