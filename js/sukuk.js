...
function renderDashboard() {
  ...
  // ══════════════════════════════════════════════════════════════════
  // العائد السنوي المرجَّح — بـ«ريال × سنة» لا بالريال وحده
  // ------------------------------------------------------------------
  // كان الترجيح بالمبلغ فقط: `Σ(المبلغ × العائد) ÷ Σ(المبلغ)`. وهذا يفترض
  // تساوي المُدد — وهو بالضبط ما أُدخل هذا المقياس لمعالجته.
  //
  // العرف في محافظ الدخل الثابت: الترجيح بـ**رأس المال × الزمن**
  // (ريال-سنة)، فريالٌ مستثمر ثلاثة أشهر لا يزن مثل ريالٍ مستثمر سنتين.
  //   العائد السنوي = إجمالي الربح ÷ Σ(المبلغ × المدة بالسنوات)
  //
  // قياس فعلي: 5,000 بـ8% لثمانية عشر شهراً + 20,000 بـ14% لثلاثة أشهر ⇒
  // المعروض **12.80%** والصحيح **10.40%**. والصفحة كانت تعرض بنفسها
  // «متوسط العائد على كامل المدة» و«متوسط المدة» بجواره — وحاصلهما يعطي
  // الرقم الصحيح ويناقض المعروض فوقه مباشرة.
  // ══════════════════════════════════════════════════════════════════
  const capitalYears = active.reduce(
    (s, o) => s + (+o.amount || 0) * ((+o.duration || 0) / 12), 0);
  const weightedAnnualPct = capitalYears > 0
    ? (totalNetProfit / capitalYears * 100)
    : 0;
  ...
  if (editOppId) {
    ...
    if (status === 'مغلق' && prevStatus !== 'مغلق') {
      ...
      if (Math.abs(diff) > tol) {
        ...
        // ⚠️ `expectedSAR` هو **رأس المال + الربح**، لا «العائد». تسميتُه عائداً
        // كانت تجعل صكّاً 1,000 بـ5% يُقارَن بـ1,050 بينما عائده 50.
        // ويصحّ هذا التطابق للصكّ الذي يُطفأ أصله ضمن التوزيعات؛ أمّا ذو الدفعة
        // الختامية — حيث تُسجَّل القسائم وحدها — فيظهر «ناقص» بمقدار رأس المال
        // كاملاً في كل مرة، فيُذكَر البديل صراحةً بدل تركه لغزاً.
        const msg =
          `⚠️ تحذير — عدم تطابق التوزيعات\n\n` +
          `الإجمالي المتوقع عند الاستحقاق: ${formatSAR(expectedSAR)}\n` +
          `منه ربح:                        ${formatSAR(expectedSAR - amount)}\n` +
          `المحصّل فعلياً:                 ${formatSAR(paidSAR)}\n` +
          `الفرق (${shortOrOver}): ${formatSAR(Math.abs(diff))}\n\n` +
          `إن كان هذا صكّاً يردّ أصله دفعةً ختامية ولم تسجّل إلا القسائم، فالفرق ` +
          `يقارب رأس المال (${formatSAR(amount)}) وهو متوقَّع لا خطأ.\n\n` +
