...
async function exportMonthlyReviewMD() {
  ...
  try {
...

    {
      ...
      // ── سجلّ تغييرات السجلات الشهرية ─────────────────────────────
      // ⚠️ كان يُكتب في `store.audit` عند كل تعديل وحذف (ومع التحديد
      // الجماعي: قيدٌ منفصل لكل شهر)، ويدخل النسخة الاحتياطية كاملاً،
      // **ولا يُقرأ في التقرير إطلاقاً**. وهو تاريخ تعديلاتك على دخلك —
      // مصدرُ إجاباتِ «متى تغيّر هذا الرقم ولماذا؟».
      {
        const _sa = Array.isArray(salaryData.audit) ? salaryData.audit : [];
        h3('14.1 سجلّ تغييرات السجلات الشهرية');
        if (_sa.length) {
          const _actAr = { edit: 'تعديل', delete: 'حذف', add: 'إضافة' };
          const recent = _sa.slice().reverse();     // الأحدث أولاً
          p(mdTable(['التاريخ', 'الإجراء', 'الشهر', 'ما تغيّر'],
            recent.map(e => {
              const mo = (e.month && e.year)
                ? `${MONTHS[(+e.month || 1) - 1]} ${e.year}` : '—';
              const ch = Array.isArray(e.changes) && e.changes.length
                // الحقول كما يكتبها `_auditDiff` في salary.js: field · from · to
                ? e.changes.map(c => `${c.field}: ${c.from ?? '—'} ← ${c.to ?? '—'}`).join(' · ')
                : (e.action === 'delete' ? 'حُذف السجل كاملاً' : '—');
              return [(e.at || '').slice(0, 10) || '—', _actAr[e.action] || e.action || '—', mo, ch];
            })));
          const _del = _sa.filter(e => e.action === 'delete').length;
          p(`**${_sa.length}** قيداً${_del ? ` — منها **${_del}** حذفاً` : ''}. `
          + 'التعديل الذي لا يغيّر شيئاً لا يُسجَّل، والسجلّ يُقصّ من أوّله عند بلوغ سقفه.');
        } else {
          p('_لا تغييرات مسجَّلة._ يُسجَّل قيد عند كل تعديل أو حذف لسجلّ شهري.');
        }
      }
    }
    hr();
...
`
    ...
    // ════════════════════════════════════════════════════════
    // 40. السجلات الدستورية — التاريخ الذي تُبنى عليه القرارات
    // ════════════════════════════════════════════════════════
    // ⚠️ الأربعة كانت **تُحفَظ وتُنسَخ وتُستعاد ولا تُقرأ في التقرير**
    // إطلاقاً. وهي بطبيعتها **تاريخية**: القراءات المؤكِّدة (م.43) وسجلّ
    // التدقيق (م.72) وتاريخ الفئات (م.26) وعمق التوزيع (م.41). ومستندُ
    // دورةٍ لا يحمل تاريخ قراراته لا يصلح مرجعاً لمراجعتها (م.71 و72).
    book('A');
    await tick('السجلات الدستورية');
    h2('40. السجلات الدستورية — تاريخ القرار لا لقطته');
    p('أربعة سجلات يكتبها محرّك القرار عبر الزمن ويقرؤها في كل تشغيل. تُخزَّن في `user_settings` وتدخل النسخة الاحتياطية كاملةً.');
    {
      const _cd = async k => { try { return await syncedGet(k, null); } catch (_) { return null; } };

      // ── 40.1 القراءات المؤكِّدة (م.43) ──
      h3('40.1 القراءات المؤكِّدة عبر الأرباع (readings_log_v1 — م.43)');
      p('«لا إشارة تُنفَّذ من قراءة واحدة إلا القاطعة». كل إشارة تُسجَّل مرة واحدة لكل ربع، وتكرار التشغيل في الربع نفسه لا يزيد العدّاد.');
      const _rd = await _cd('readings_log_v1');
      const _rdTk = (_rd && typeof _rd === 'object') ? Object.keys(_rd) : [];
      if (_rdTk.length) {
        const rows = [];
        _rdTk.forEach(tk => {
          const bySig = _rd[tk] || {};
          Object.keys(bySig).forEach(sig => {
            const arr = Array.isArray(bySig[sig]) ? bySig[sig] : [];
            if (!arr.length) return;
            const periods = arr.map(r => r.period || (r.date || '').slice(0, 7)).filter(Boolean);
            rows.push([tk, sig, String(arr.length),
                       periods.join(' · '),
                       (arr[arr.length - 1] || {}).date || '—']);
          });
        });
        if (rows.length) {
          p(mdTable(['الرمز', 'الإشارة', 'عدد القراءات', 'الأرباع المسجَّلة', 'آخر قراءة'], rows));
          p(`**${rows.length}** إشارة مُتابَعة على **${_rdTk.length}** رمزاً.`);
        } else { p('_السجل موجود وفارغ من القراءات._'); }
      } else {
        p('_لا قراءات مسجَّلة بعد._ تُسجَّل تلقائياً عند كل تشغيل لمحرّك القرار.');
        noteMissing('القراءات المؤكِّدة (م.43)', 'فارغة — لا إشارة متوسطة أو قوية تنتظر تأكيداً.');
      }

      // ── 40.2 سجلّ التدقيق (م.72) ──
      h3('40.2 سجلّ التدقيق — كل إشارة صدرت (audit_log_v1 — م.72)');
      p('م.72: «كل إشارة تُسجَّل بـ: التاريخ | الرمز | المادة المنطبقة | البيانات المستخدمة ومصادرها | القرار | ما إذا نُفِّذ».');
      const _au = await _cd('audit_log_v1');
      const _auArr = Array.isArray(_au) ? _au : [];
      if (_auArr.length) {
        // الأحدث أولاً — والسجل يُكتب تصاعدياً ويُقصّ من أوّله عند السقف
        const recent = _auArr.slice().reverse();
        p(mdTable(['التاريخ', 'الرمز', 'المادة', 'الإشارة', 'القرار', 'نُفِّذ؟', 'المدخلات ومصادرها'],
          recent.map(e => [
            (e.ts || '').slice(0, 10) || '—',
            e.ticker || '—',
            e.article || '—',
            ...
            e.decision || '—',
            e.executed === true ? '✅' : '◻',
            (Array.isArray(e.inputs) && e.inputs.length)
              ? e.inputs.map(x => `${x.name}=${x.value}${x.tag ? ` [${x.tag}]` : ''}`).join(' · ')
              : '—',
          ])));
        const _byArt = {};
        _auArr.forEach(e => { const a = e.article || '—'; _byArt[a] = (_byArt[a] || 0) + 1; });
        p(`**${_auArr.length}** قيداً مسجَّلاً. التوزّع على المواد: `
          + Object.entries(_byArt).sort((a, b) => b[1] - a[1])
              .map(([a, n]) => `${a} (${n})`).join(' · '));
        const _exec = _auArr.filter(e => e.executed === true).length;
        p(`**نُفِّذ منها:** ${_exec} من ${_auArr.length}`
          + (_exec < _auArr.length
              ? ` — و**${_auArr.length - _exec}** قيداً صدر ولم يُنفَّذ (ليس خطأً بالضرورة: الإشارة قد تنتظر تأكيداً أو يكون القرار «احتفظ»).`
              : '.'));
      } else {
        p('_لا قيود بعد._ يُسجَّل قيد لكل قرار يصدره المحرّك، مرة واحدة في اليوم للقرار نفسه.');
        noteMissing('سجلّ التدقيق (م.72)', 'فارغ — شغّل صفحة محرّك القرار مرة واحدة ليبدأ التسجيل.');
      }

      // ── 40.3 تاريخ الفئات (م.26) ──
      h3('40.3 تاريخ الفئات ونطاق التعليق (category_history_v1 — م.26)');
      p('الترقية أو التنزيل يحتاج تجاوز العتبة بـ±15% لمدة **دورتين متتاليتين**. هذا السجل يحمل الفئة المستقرّة والمعلَّقة وعدّاد الدورات.');
      const _ch = await _cd('category_history_v1');
      const _chTk = (_ch && typeof _ch === 'object') ? Object.keys(_ch) : [];
      if (_chTk.length) {
        p(mdTable(['الرمز', 'الفئة المستقرّة', 'فئة معلَّقة', 'دورات متتالية', 'آخر دورة مسجَّلة'],
          _chTk.map(tk => {
            const h = _ch[tk] || {};
            return [tk, h.settled || '—', h.pending || '—',
                    h.streak != null ? String(h.streak) : '—', h.lastCycle || '—'];
          })));
        const _pend = _chTk.filter(tk => (_ch[tk] || {}).pending).length;
        if (_pend) p(`⏳ **${_pend}** رمزاً في نطاق التعليق — لم يكتمل شرط الدورتين بعد.`);
      } else {
        p('_لا تاريخ فئات بعد._ يُكتب عند كل تشغيل للمحرّك، دورةً واحدة لكل ستة أشهر.');
        noteMissing('تاريخ الفئات (م.26)', 'فارغ — لا مقارنة عبر الدورات ممكنة حتى يتراكم.');
      }

      // ── 40.4 عمق تاريخ التوزيع (م.41) ──
      h3('40.4 عمق تاريخ التوزيع المُدخَل يدوياً (dividend_depth_v1 — م.41)');
      p('«ممنوع الحكم بفشل الاستدامة قبل استخراج التوزيعات المدفوعة فعلياً لأربع سنوات». هذه سنوات أدخلها المالك يدوياً حين لا يكفي سجلّ الأرباح.');
      const _dp = await _cd('dividend_depth_v1');
      const _dpTk = (_dp && typeof _dp === 'object') ? Object.keys(_dp) : [];
      if (_dpTk.length) {
        p(mdTable(['الرمز', 'سنوات مُدخَلة يدوياً', 'الحدّ الدستوري'],
          _dpTk.map(tk => [tk, String(_dp[tk] ?? '—'), '4 سنوات'])));
      } else {
        p('_لا مُدخَلات يدوية._ المحرّك يستنبط العمق من سجلّ أرباحك وملفات تداول وحدها.');
      }
    }
    hr();
