...
// ══════════════════════════════════════════════════════════════════════
// أول سنة في وعاء العوائد — تُقرأ منها حالة الكسر الهيكلي بدل كتابتها نصّاً
const TASI_POOL_FROM_YEAR = 2004;
// ⚠️ `window.CARD_INFO` كائنٌ حرفيّ يُبنى **وقت التحليل**، فأي اسم يُقرأ
// داخل قوالبه ولم يُعرَّف بعدُ يُسقط الملف كلّه لا البطاقة وحدها — وهو ما
// وقع فعلاً مع `TASI_PRICE_YE`. و`STRUCTURAL_BREAK_LABEL` يأتي من ملف آخر
// (constitution.js) المحمَّل قبله، لكن `typeof` لا يرمي أبداً فتبقى البطاقة
// سليمة حتى لو تغيّر ترتيب التحميل يوماً.
const _BREAK_LBL = (typeof STRUCTURAL_BREAK_LABEL === 'string')
  ? STRUCTURAL_BREAK_LABEL : 'أكتوبر 2016 — رؤية 2030';
const TASI_PRICE_YE = [
...
window.CARD_INFO = {
  ...
  'montecarlo': {
    ...
    body: `
      ...
      <p class="info-note">📐 <strong>نافذة البيانات تعبر كسراً هيكلياً:</strong> وعاء العوائد هنا يمتدّ
      <strong>${TASI_POOL_FROM_YEAR}–2024</strong>، و<strong>${_BREAK_LBL}</strong> يحمل كسراً هيكلياً مؤكَّداً بـBai–Perron
      تنخفض بعده حساسية تاسي لأسعار النفط بوضوح وترتفع أهمية العوامل النقدية (Abid 2026).
      فأي <em>علاقة بنيوية</em> تُقدَّر عبر هذه النافذة متوسّطُ حالتين لا وصفُ واحدة.<br>
      <strong>ولماذا لا نقصّ التاريخ عند 2016 رغم ذلك:</strong> الكسر في علاقة تاسي بالنفط والمتغيّرات
      الكلّية، لا في كون العائد عائداً. وقصُّ الوعاء إلى ما بعد 2016 يُسقط انهيارَي <strong>2006</strong>
      و<strong>2008</strong> — أي يحذف أسوأ ما مرّ بالسوق فيجعل توزيع المخاطر <strong>أكثر تفاؤلاً</strong>،
      وهو عكس غرض المحاكاة. القاعدة هنا: <strong>يُعلَن الكسر ولا يُحذف التاريخ</strong>.</p>
...
function runMonteCarlo() {
  ...
  if (statusEl) {
    statusEl.textContent = `${N.toLocaleString('ar-SA')} مسار · كتل ${MC_BLOCK_YEARS} سنوات`
      + ` · عوائد تاسي ${TASI_POOL_FROM_YEAR}–2024${
          spansStructuralBreak(TASI_POOL_FROM_YEAR).spans
            ? ` ⚠️ تعبر كسر ${STRUCTURAL_BREAK_YEAR}` : ''}`
      + ` · نمو توزيع ${(DIV_GROWTH_WEIGHTED * 100).toFixed(2)}% (م.7)`
...
function _renderMonteCarlo(r) {
  ...
  // مقدّمة مطويّة: البطاقة تعرض النتيجة، وشرحُ الطريقة خلف نقرة.
  const intro = foldHtml('وش سوّينا هنا؟', `🎲 <strong>وش سوّينا هنا؟</strong> شغّلنا "مستقبل محفظتك" <b>${r.N.toLocaleString('ar-SA')} مرة</b>، كل مرة بعوائد سوق مختلفة
      ...
      نسحب <b>كتلاً متتابعة بطول ${r.blockYears} سنوات</b> لا سنوات مبعثرة، حتى يبقى تتابع السوق الحقيقي (انهيار يتبعه تعافٍ) محفوظاً.`, { icon: '🎲' });
