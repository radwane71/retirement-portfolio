...
// ══════════════════════════════════════════════════════════════════════
{
  ...
  t('طبقة الذكاء تستعمل نطاقات م.48',
    /valueBandOf\(r\.price \/ r\.fairValue, r\.fvCV/.test(di)
    && !/gates\.push\('السعر فوق قيمته العادلة'\)/.test(di),
    'كانت تستبعد أي هامش سالب — والخطة تحتها تُصدر أمر شراء للسهم نفسه');

  // الاسم يجب أن يطابق ما يضعه المحرّك على الصف، وإلا مُرِّر undefined
  // فتعطّل توسيع نطاقات م.39 عند التشتّت العالي بلا أي أثر ظاهر.
  t('اسم حقل التشتّت واحد في المحرّك والذكاء',
    /fvCV: val \? val\.cv : null/.test(de2) && /r\.fvCV/.test(di) && !/r\.dispersionCV/.test(di),
    'المحرّك يسمّيه fvCV — أي اسم آخر في الذكاء يُمرَّر undefined صامتاً');

  // التشتّت كسرٌ لا نسبة: طباعته بلا ×100 تقول «0%» لتشتّت 30%
  t('نصّ التشتّت يضرب في 100 قبل العرض',
    /formatNum\(base\.fvCV \* 100, 0\)/.test(de2),
    'fvCV مقسوم على 100 عند القراءة، فطباعته كما هي تُظهر صفراً');
}
