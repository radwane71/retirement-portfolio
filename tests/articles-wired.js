...
// ═══ م.41 — البوابة تسبق الحكم بالفشل، لا تُستدعى بعده ═══
{
  ...
  // الوسم انقسم إلى بوابتين متمايزتين (2026-08-27): 'م.41-عمق' لعمق التاريخ
  // و'م.41-دوري' لنقاط الدوري/البنيوي — توحيدهما كان يطبع السبب الخطأ.
  t('وتُرجع watch لا fail عند النقص', has(/status: 'watch', gatedBy: 'م\.41-عمق'/), true);
  t('وبوابتا م.41 متمايزتان', has(/gatedBy: 'م\.41-دوري'/), true);
  t('ولا وسم م.41 مجرَّد باقٍ', !/gatedBy: 'م\.41'[,\s]/.test(DE), true);
...
// ═══ م.45 — بوابة الخسارة المحققة تعترض الخروج ═══
{
  ...
  // انتقل إلى التفصيل المطويّ بعد إعادة بناء السطر — معروض لا محذوف.
  t('والتعادل الحقيقي معروض في التفصيل',
    has(/التعادل الحقيقي /) && has(/formatNum\(o\.breakEven\)/), true);
}
...
{
  // أُعيد بناء السطر مطويّاً (2026-08-27) بعد بلاغ المالك «كأنه كتاب ومقال»:
  // حكمٌ واحد ظاهر — كاسب/خاسر بالرقم والنسبة — والقرار النهائي صريح، وكل
  // ما عداه خلف نقرة. الحارس صار على البناء لا على الصياغة القديمة.
  t('السطر مطويّ في عنصر details', /<details class="de-order">/.test(DE), true);
  t('والمحصّلة تشمل الخروج المؤجَّل',
    /const isSell = kind === 'exit' \|\| kind === 'trim' \|\| kind === 'deferredExit'/.test(DE), true);
  t('والقرار النهائي صريح لا مُستنتَج',
    /لا تخرج الآن/.test(DE) && /تقدر تخرج/.test(DE), true);
  t('والتفصيل خلف نقرة لا في السطر',
    /class="de-o-body"/.test(DE) && /متوسط تكلفتك/.test(DE), true);
